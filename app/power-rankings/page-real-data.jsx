...
export default function PowerRankingsPage() {
  ...
  // Historical team rankings data from the API
  const [historicalRanks, setHistoricalRanks] = useState({});
  ...
  useEffect(() => {
    ...
    // Create confetti celebration for first visit - reduced particle count for performance
  ...
  // Function to create confetti celebration effect - reduced for performance
  const createConfetti = () => {
    const colors = ['#FCD34D', '#60A5FA', '#F472B6', '#A78BFA', '#34D399'];
    const container = document.body;
    
    // Create only 20 confetti pieces (reduced from 50)
    for (let i = 0; i < 20; i++) {
      ...
      // Remove after animation is complete - faster cleanup
      setTimeout(() => {
      ...
      try {
          ...
          if (cachedData && cachedTimestamp) {
            ...
            if (cacheAge < 24 * 60 * 60 * 1000) {
              ...
              if (data.data && data.data.history) {
                setHistoricalRanks(data.data.history);
              }
              setFetchStatus({
                fromCache: true,
                cachedAt: parseInt(cachedTimestamp, 10),
                fetchError: null,
                nextUpdateAt: null, // Will update this when we have fresh data
                lastScheduledUpdate: null
              });
              setIsLoading(false);
        ...
        if (!useCachedData) {
          ...
          if (response.ok) {
            ...
            // Set historical ranks data if available
            if (data.data && data.data.history) {
              setHistoricalRanks(data.data.history);
            }
  ...
  // Helper to determine team trend (for visual indicators) - now using real data
  const getTeamTrend = (teamName) => {
    if (!historicalRanks[teamName] || activeWeek <= 1) return 'stable';
    
    // Use real history data from the API
    const history = historicalRanks[teamName];
    ...
    // Check if we have valid data for both weeks
    if (history[currentWeekIndex] === null || history[prevWeekIndex] === null) return 'stable';
  ...
  // Card hover state is now handled by CSS for better performance
  const handleCardHover = (elementId, isHovered) => {
    // This is a lightweight alternative that we can use to add any
    // JavaScript-based effects that won't cause performance issues
    const card = document.getElementById(elementId);
    if (card && isHovered) {
      // Add light glow effect on hover using CSS classes instead of JS transforms
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-blue-950 text-white relative overflow-hidden" ref={containerRef}>
                          ...
                          <div className="flex items-center gap-3 mb-2">
                            <div className="w-8 h-8 rounded-full flex items-center justify-center overflow-hidden border border-white/10 bg-gradient-to-br from-blue-900/50 to-purple-900/50">
                              <Image
                                ...
                                onError={(e) => handleImageError(e, team.team, 'small')}
                              />
                            </div>
                            <span className="font-medium text-sm">{team.team}</span>
                          </div>
                          
                          {/* History Chart - showing real historical data */}
                          <div className="history-chart">
                            {historicalRanks[team.team] ? (
                              historicalRanks[team.team].map((rank, weekIndex) => {
                                // Skip null/undefined ranks or future weeks
                                if (rank === null || weekIndex + 1 > activeWeek) {
                                  return (
                                    <div
                                      key={`chart-placeholder-${team.team}-${weekIndex}`}
                                      className="history-bar history-bar-animate"
                                      style={{ 
                                        height: '10px',
                                ...
                                return (
                                  <div
                                    ...
                                    data-week={`W${weekIndex + 1}`}
                                    data-rank={`#${rank}`}
                                    aria-label={`${team.team} ranked #${rank} in week ${weekIndex + 1}`}
                                  >
                                    {weekIndex + 1 === activeWeek && (
                                      <div className="history-bar-logo">
                                        <Image
                                          src={`/logos/${team.team}.png`}
                                          alt={team.team}
                                          width={16}
                            ...
                            <div
                              ...
                              data-rank={`#${index + 1}`}
                            >
                              {/* Team logo in current week indicator */}
                              <div className="history-bar-logo">
                                <Image
                                  src={`/logos/${team.team}.png`}
                                  alt={team.team}
                      ...
                      {powerRankings?.data?.teams?.map((team, index) => {
                        ...
                        return (
                          <motion.div 
                            ...
                            onMouseEnter={() => handleCardHover(`team-card-${index}`, true)}
                            onMouseLeave={() => handleCardHover(`team-card-${index}`, false)}
                          >
                      ...
                      {powerRankings?.data?.players?.map((player, index) => {
                        ...
                        return (
                          <motion.div 
                            ...
                            onMouseEnter={() => handleCardHover(`player-card-${index}`, true)}
                            onMouseLeave={() => handleCardHover(`player-card-${index}`, false)}
                          >
                            {/* Achievement badge */}
                            {achievement && (
      ...
      {/* Data Timestamp and Update Schedule */}
            ...
            <div className="inline-flex items-center bg-blue-900/30 px-4 py-2 rounded-lg border border-blue-500/20">
              ...
              <div className="flex flex-col items-start">
                ...
                <span className="text-xs text-blue-300/70">
                  {fetchStatus.lastScheduledUpdate ? (
                    <>Data refreshed at {new Date(fetchStatus.lastScheduledUpdate).toLocaleString('en-US', {
                      hour: 'numeric',
                      minute: '2-digit',
                      hour12: true,
            ...
            {fetchStatus.nextUpdateAt && (
              <div className="text-xs text-blue-300/70">
                Next scheduled update: {
                  new Date(fetchStatus.nextUpdateAt).toLocaleString('en-US', {
                    month: 'short',
                    day: 'numeric',
                    hour: 'numeric',
                    minute: '2-digit',
